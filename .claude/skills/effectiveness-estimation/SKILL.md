...
10. **Attribution line: model name only** - The opening `_The following analysis was done on [date] by [model], with prompts from Impact List staff._` line names the model **plainly**. Never append a context-window size or reasoning-effort qualifier (no `(1M context)`, `(Extra High)`, `(Max)`, `(High)`, etc.), and don't add or restore one when editing a page.
