...
- A level-1 `# Internal Notes` heading and everything under it is stripped from published content — but ONLY that exact heading (`## Internal Notes` or different casing would ship publicly; tightening this is review item 10).
- `{{VARIABLE}}` placeholders (e.g. `{{CONTRIBUTION_NOTE}}`) are substituted by the generator from `MARKDOWN_VARIABLES`.
