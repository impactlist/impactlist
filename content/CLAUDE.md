...
- **Attribution line:** a page may open with `_The following analysis was done on [date] by [model], with prompts from Impact List staff._`. Name the model **plainly** — never append a context-window size or reasoning-effort qualifier (no `(1M context)`, `(Extra High)`, `(Max)`, `(High)`, etc.), and don't add or restore one when editing a page.
