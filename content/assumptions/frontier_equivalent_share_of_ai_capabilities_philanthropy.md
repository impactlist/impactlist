...
Industry now drives the frontier almost entirely. A philanthropic dollar enters an ecosystem already saturated with private capital and strong commercial incentives. In many cases the work would mostly happen anyway. So it would be clearly wrong to model a dollar here as **\$1 of fully additional frontier progress**.
...
Even though private capital dominates, the frontier effect is not negligible. This cause area includes projects genuinely close to the frontier — direct frontier labs, university labs doing capability-relevant research, open-weight or open-infrastructure projects, and the datasets, benchmarks, tools, and research that quickly spill over into frontier practice. Open or academic work can accelerate the frontier even when it is not literally a frontier training run. So a capability-relevant donation here can matter more for frontier progress than its budget line alone would suggest.
...
Our plausible range is **0.12–0.55**. Two largely independent axes of uncertainty drive its width. First, **where the marginal dollar lands** — loosely-tied public-interest work at one end, frontier-direct or open-model work at the other. Second, **how much of that work is crowded out versus absorbed by the frontier** — whether the private sector would have produced it anyway, or whether outputs spill quickly into frontier systems. The interval is wide for three reasons: both axes are genuinely uncertain; they tend to move together with your overall read of the cause; and the quantity itself is an unobservable inference rather than a measured figure.
...
Treating those two end-members as the published plausible range would overstate our confidence in the extremes: it would require both the destination of the dollar and the crowd-out/absorption rate to land at their favorable (or unfavorable) limits together. So we pull each end partway in from its scenario limit, which gives **0.12–0.55**. That range still widens well beyond what fully independent axes would give, because the two axes are positively correlated and the whole quantity carries heavy structural uncertainty.
:::
...
If you think this cause is basically direct frontier acceleration, raise this number.
If you think the relevant charities are mostly public-interest AI, with weak frontier spillovers, lower it.
