...
The 0.5 term assumes the welfare range is roughly symmetric around a neutral point, so the negative portion makes up half the total range. This is a modeling convention, not a claim that hens spend literally half their lives below neutral: once a total welfare range is specified and there is no strong argument that neutral sits far from the midpoint, a symmetric split is the cleanest way to map "fraction of negative welfare removed" into the total range. If the negative portion were larger or smaller than half, the final QALY conversion would move proportionally.

### 3. Why use 25% for the cage-to-aviary improvement?

This is the key judgment call. Following Fischer's DALY-conversion framework, we express the reform as the share of a hen's negative welfare range that it removes — so the question is not whether aviaries are good in absolute terms, but how much better they are than conventional cages. ([Fischer 2023](https://www.law.georgetown.edu/public-policy-journal/wp-content/uploads/sites/23/2024/02/Bob-Fischer.pdf))
