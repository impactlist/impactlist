...
effects:
  - effectId: population
    ...
    costPerMicroprobability: 1_200_000
...
- **Cost per microprobability:** \$1.2 million (conditional positive-effect range: \$200,000–\$70 million; lower tail includes net harm)
...
1. The baseline probability of AI-caused existential catastrophe this century is approximately 14%, with a plausible range around 3–35%. A useful decomposition is about 10% extinction risk plus about 4 percentage points of additional irreversible-disempowerment or lock-in risk. ([See detailed justification](/assumption/ai-existential-catastrophe-probability)) {{CHALLENGE_ASSUMPTION:1}}
2. AGI — machines that can do any mental task better and more cheaply than humans — most likely arrives around 2036, with a plausible range of roughly 2030–2050. That is roughly when catastrophic AI risk becomes pressing. ([See detailed justification](/assumption/timelines-to-agi)) {{CHALLENGE_ASSUMPTION:2}}
...
4. Raw cumulative AI safety spending through 2025 is roughly \$2.6 billion (~\$1.2 billion non-lab — philanthropy, government safety institutes, academia — plus ~\$1.4 billion of frontier-lab internal safety-labeled work). Because a frontier-lab safety dollar is less effective than a serious non-lab dollar, we weight the lab pool at about 0.3, giving roughly \$1.6 billion of non-lab-equivalent spending — the relevant denominator for marginal donations. ([McAleese](https://forum.effectivealtruism.org/posts/XdhwXppfqrpPL2YDX/an-overview-of-the-ai-safety-funding-situation), [Coefficient Giving](https://coefficientgiving.org/research/ai-safety-and-security-need-more-funders/)) {{CHALLENGE_ASSUMPTION:4}}
5. Historical AI safety spending has probably reduced AI-caused existential-catastrophe risk by about 0.13 percentage points, with a plausible range around -0.04 to 0.5 percentage points. ([See detailed justification](/assumption/effect-of-all-ai-safety-spending-on-ai-existential-catastrophe)) {{CHALLENGE_ASSUMPTION:5}}
...
The point estimate (\$1.2 million per microprobability) comes from a field-level estimate of what historical AI safety spending has already achieved. Per Assumptions 4 and 5, about \$1.6 billion of effective spending has probably reduced AI-caused existential-catastrophe risk by about **0.13 percentage points**:

- Microprobabilities averted: $0.0013 / 10^{-6} = 1{,}300$
- Cost per microprobability: $$\$1.6\text{B} / 1{,}300 \approx \$1.2\text{ million}$$

Because the field-level risk-reduction range crosses zero, the full distribution does not convert into a finite positive cost range. Conditional on the net effect being positive, weaker or stronger assumed effects together with denominator uncertainty give a range of about **\$200,000 to \$70 million** per microprobability; the lower tail is net harm rather than just high-cost benefit.
