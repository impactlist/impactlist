...
1. Donations to high-impact climate nonprofits avert roughly 1.5 tCO₂e per \$1 in expectation, with a plausible range of roughly 0.3–7 tCO₂e/\$. Here tCO₂e means "tons of carbon-dioxide equivalent": one shared unit for warming impact across gases such as CO₂ and methane. The published sources span a wide band. Giving Green's current rule of thumb is ~1 tCO₂e/\$. Its detailed Evergreen model produced ~1.9 tCO₂e/\$ in its realistic case and ~2.4 tCO₂e/\$ at the median. And Founders Pledge's 2023 high-impact climate grants imply roughly 6.9 tCO₂e/\$ (derived from their reported 47.9 MtCO₂e averted on \$6.9M granted). Climate-philanthropy results depend heavily on policy windows and tractable opportunities, so we use 1.5 tCO₂e/\$ as a middle-of-the-road portfolio estimate rather than anchoring to either the roughest rule of thumb or the strongest historical year. ([Giving Green 2025](https://www.givinggreen.earth/post/eager-to-measure-the-impact-of-your-climate-donation-forget-offset-math-focus-on-systems-change), [Giving Green Evergreen CEA](https://www.givinggreen.earth/research/evergreen-collaborative-top-climate-nonprofit-evaluation), [Founders Pledge 2023 Impact](https://impact.founderspledge.com/2023/climate-change)) {{CHALLENGE_ASSUMPTION:1}}
2. Each additional ton of CO₂ emitted today causes approximately 2.26 × 10⁻⁴ expected deaths between 2020 and 2100, i.e. one expected death per ~4,425 tons. This estimate is stated per ton of **CO₂ itself**, not per tCO₂e, which is why Assumption 4 has to explain how we translate from a mixed greenhouse-gas portfolio into this mortality estimate. ([Bressler 2021](https://www.nature.com/articles/s41467-021-24487-w), [Carleton et al. 2022](https://academic.oup.com/qje/article/137/4/2037/6571943)) {{CHALLENGE_ASSUMPTION:2}}
3. Each climate-attributable death corresponds to roughly 5 QALYs lost on average, with a plausible range of 2–10. A European heat-mortality study adjusts for harvesting and finds about 5,907 years of life lost from ~2,234 heat deaths per year, or about 2.6 years per death, in a relatively old population. (Harvesting is short-term mortality displacement: some heat deaths occur among people who would have died within days or weeks regardless.) But [Wilson et al. 2024](https://www.science.org/doi/10.1126/sciadv.adq3367) find that in Mexico, people under 35 account for 75% of heat-related deaths and 87% of heat-related lost life years. So in hotter settings heat mortality can fall much more heavily on younger people than the older European evidence implies. Wilson's lost-life-years measure rests on remaining life expectancy rather than harvesting-adjusted QALYs, so we update the central estimate only modestly to 5 rather than much higher. ([Baccini et al. 2013](https://journals.plos.org/plosone/article?id=10.1371%2Fjournal.pone.0069638), [Wilson et al. 2024](https://www.science.org/doi/10.1126/sciadv.adq3367)) {{CHALLENGE_ASSUMPTION:3}}
4. To combine Assumptions 1 and 2, we treat avoided tCO₂e as roughly comparable to avoided tCO₂ in a simple century-scale portfolio model. This is imperfect: tCO₂e is a warming-equivalent unit across gases, while the mortality estimate in Assumption 2 is written per ton of actual CO₂. Methane-heavy interventions likely produce faster temperature and ozone benefits than this model captures, so the approximation may understate some methane-focused work. CO₂-heavy portfolios fit the mortality estimate more directly. The published range explicitly leaves room for this gas-mix/unit-conversion uncertainty. ([Giving Green 2025](https://www.givinggreen.earth/post/eager-to-measure-the-impact-of-your-climate-donation-forget-offset-math-focus-on-systems-change), [IPCC AR6 WG1 Chapter 6](https://www.ipcc.ch/report/ar6/wg1/chapter/chapter-6/)) {{CHALLENGE_ASSUMPTION:4}}
5. It takes approximately 5 years for donations to translate into enacted policy, scaled deployment, or market shifts that meaningfully change emissions trajectories. {{CHALLENGE_ASSUMPTION:5}}
6. Benefits are modeled over a window of 80 years, approximately matching the time span of the main mortality literature (which runs through about 2100) while allowing some spillover beyond it, and staying within the site's global time-limit parameter. {{CHALLENGE_ASSUMPTION:6}}

## Details

### Cost per QALY

We model climate-mitigation philanthropy as:
...
The result swings most on $E$ (tons averted per dollar) and, secondarily, $L$ (QALYs lost per death); $M$ is comparatively well-anchored. Published estimates of $E$ vary widely — roughly 1–7 tCO₂e/\$ across the sources in Assumption 1. That spread drives our wide plausible range of roughly **\$40-\$15,000/QALY**.

We publish a range that is *wider* than what swinging the headline parameters produces, because the largest uncertainties here sit partly outside those parameters. Pushing $E$ and $L$ to their favorable and unfavorable edges together gives only about \$60-\$7,400/QALY. But that holds $M$ (the mortality cost of carbon) fixed, even though the underlying social-cost-of-carbon literature disagrees substantially. It also ignores the rough tCO₂e-to-CO₂ approximation of Assumption 4. And — most important on the expensive side — it assumes the systems-change mechanism works roughly as modeled. If much of the averted emissions would have happened anyway (a weak funding counterfactual), effective $E$ falls well below 0.3 and the cost per QALY climbs past the parameter sweep. That is why the upper bound sits at \$15,000 rather than \$7,400.
...
The 80-year duration reflects a compromise between the mortality literature and the site's global time-limit parameter, which truncates effects beyond it. The main climate-mortality studies used here focus on harms through 2100. This estimate is being made in 2026 with a 5-year start time, so an 80-year duration roughly corresponds to benefits accruing from about 2031 to 2111 — somewhat beyond 2100, but not indefinitely far into the future.

This duration leaves out some longer-run effects: CO₂ persists in the atmosphere for centuries, and some climate impacts continue beyond 2100. ([IPCC AR6 WG1 Technical Summary](https://www.ipcc.ch/report/ar6/wg1/chapter/technical-summary/))
