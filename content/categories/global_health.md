...
This effect captures welfare gains from donations to exceptionally cost-effective global-health charities, especially GiveWell-style programs that prevent child deaths from malaria, vitamin A deficiency, and vaccine-preventable disease. We model the best currently donatable global-health opportunities, not the average global-health nonprofit. These programs mostly help very young children, so the gains are dominated by many decades of additional life.
...
Those causes can still be very valuable, but they are usually less tightly evidence-backed and less cost-effective than the narrow set of top child-survival opportunities modeled here. Broad global-health recipients should therefore receive recipient-level overrides rather than inheriting this default.
:::
...
The midpoint is about **\$105/QALY**. Averaging beats either anchor alone. The strongest single published grant would overstate the average current opportunity. The funding bar alone understates GiveWell's recent top-charity portfolio, which has averaged somewhat better than the bar. The historical average is an unweighted cross-check across the four top charities, not a claim about GiveWell-directed funding weights, so the funding-bar estimate is the cleaner marginal anchor. Non-mortality benefits — reduced illness, lower treatment costs, later-life income — are inside the QALY metric here, carried by the benchmark bridge rather than added separately.
...
So our plausible range is **\$30-\$180/QALY**. We keep it wide because these models are rough. GiveWell itself emphasizes that they are used mainly for comparing grants rather than for precise cardinal measurement.

### Start time

Some benefits begin almost immediately once children receive nets, preventive malaria medicine, supplements, or vaccines. But this site's start-time field is not asking when the first pill is swallowed. It asks when a donation reliably turns into real-world impact.
...
So **1 year** is a reasonable round portfolio-average start time. It is slower than assuming instant impact, but not so slow that it misses the fact that these are direct-delivery interventions rather than long-policy-lag bets.
...
1. **How close the next marginal donor dollar really is to GiveWell's current public 6x bar.** If the best current unfunded opportunities are materially better than the bar, the estimate is too pessimistic. If you fund weaker programs than GiveWell's current top set, it is too optimistic.
