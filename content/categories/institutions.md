...
The range is now close to the pessimistic scenario corner rather than a narrow independent-parameter combination. That is intentional: an optimistic or pessimistic worldview tends to move cost, probabilities, and values together, and the sweep still omits structural uncertainty about how fat-tailed the distribution really is, how often philanthropy is genuinely causal, and how well the money-to-QALY bridge captures institutional gains. The wider top end reflects the fat-tailed downside — if major wins are rarer than 10% or the scenario values are overstated, the estimate worsens fast.
