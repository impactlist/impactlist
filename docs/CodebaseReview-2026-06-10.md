...
1. **[High] Unreachable "pulse effect" implementation contradicts its own asserts** — `src/utils/effectsCalculation.js`. `assertPositiveNumber(effect.windowLength, …)` (in `qalyEffectToCostPerLife`, `populationEffectToCostPerLife`, and `calculateCostPerLife`) throws on `windowLength === 0`, yet ~45 lines of carefully-commented `windowLength === 0` "instantaneous pulse" handling sit below it, and `validateEffectWindow` has a clause specifically for zero windows. Either pulses are an intended feature blocked by the asserts, or the most important file in the codebase contains misleading dead code. **Needs a product decision** (Elliot): delete the pulse branches, or relax the asserts to `assertNonNegativeNumber` and support them.

2. **[High] Calculator table columns silently sort garbage** — `src/components/calculator/RecipientTable.jsx` + `src/components/shared/SortableTable.jsx`. The lives/cost-per-life/category columns are sortable by default (`sortable !== false`), but the calculator's row objects don't carry those fields (values are computed in `render`), so clicking those headers compares `undefined` and quietly scrambles or no-ops while the sort arrow displays. Fix: per-column `sortable: false` or `sortValue`/`comparator` support, and make the comparator fail loudly when both values are `undefined` so the class of bug can't recur.

3. **[High] `.defaultProps` on function components is dead under React 19** — 7 components: `AssumptionsDropdown`, `AssumptionsSelector`, `ConfirmActionModal`, `ShareAssumptionsModal`, `SaveAssumptionsModal`, `SharedImportDecisionModal`, `Header`. React 19 removed `defaultProps` for function components, so none of these defaults apply. Mostly redundant with destructure defaults, but `AssumptionsDropdown`'s destructuring has *no* defaults — any future caller omitting `onRename`/`onDelete` gets a runtime TypeError instead of the intended no-op. Move defaults into destructuring and delete the blocks.

4. **[High] Unguarded `JSON.parse` of persisted storage can brick the calculator** — `src/pages/DonationCalculator.jsx` (localStorage ×2) and `src/contexts/AssumptionsContext.jsx` (sessionStorage). A corrupted value throws inside an effect → ErrorBoundary → "Refresh Page" can never fix it because the bad key persists. Fix that stays fail-loud: catch parse/normalize failures, clear the offending key, surface an explicit "saved values were corrupted and reset" notification.

5. **[High] No 404 route** — `src/App.jsx` has no `path="*"`; unknown URLs render header + empty main + footer with no message (the silent failure the project forbids). Bad entity IDs (`/donor/xyz` etc.) throw users onto the developer error screen. Bad URLs are expected input: add a NotFound route and have detail pages render it for unknown IDs.
...
8. **[Medium] Swallowed exceptions violating fail-loud** — `src/utils/effectEditorUtils.js` `calculateEffectCostPerLife` has a bare `catch { return Infinity; }` (hides genuine programming errors, not just partial input); `src/components/shared/SampleDonationCalculator.jsx` `catch { setCostPerLife(Infinity); }` renders "N/A" on *any* error. Narrow both to expected validation states; rethrow the rest (cf. `CategoryEffectEditor`, which does this correctly).

9. **[Medium] Broken-and-dead date sort** — `src/pages/RecipientDetail.jsx` sets `dateObj: donation.date` (an ISO *string*) then sorts with `b.dateObj - a.dateObj` → `NaN` comparator → no-op, while the comment claims "most recent first". `DonorDetail` does it correctly with `new Date()`. Both pre-sorts are then discarded by `SortableTable`'s default sort anyway — fix the construction, delete the misleading pre-sorts.
...
1. **[Medium] Upstream Redis error bodies leak to clients.** `upstashRedisClient.js` embeds raw Upstash response bodies in `error.message`; `handleApiError` returns 5xx messages verbatim; `api/health.js` echoes them publicly; the client shows them in notifications. Log details server-side; return generic messages for ≥500.
2. **[Medium] No caching or rate limiting on reads → free quota burn.** Snapshot GETs set no `Cache-Control` (snapshots are immutable — cache id-based ones hard, slug-based ones shorter) and have no rate limit; `GET /api/health?check=redis` costs 2 Redis commands per anonymous hit. On the Upstash free tier this lets anyone exhaust the daily quota and 503 the feature.
3. **[Medium] The 120KB streaming body cap is dead in production.** Vercel pre-parses JSON bodies (`parseJsonBody` returns `req.body`), so the real prod limit is Vercel's ~4.5MB; a content type Vercel parses to a *string* (e.g. `text/plain`) attaches listeners to an already-consumed stream and likely hangs until the platform timeout. Handle string/Buffer `req.body`, reject non-JSON content types with 415. (Cross-origin `text/plain` "simple" POSTs also spend rate-limit quota — an Origin/Content-Type check closes that.)
4. **[Medium] The production monitoring plan can't work as written.** `docs/SharedAssumptionsProductionPlan.md` says to watch logs for `rate_limited`/`invalid_reference`, but `handleApiError` only logs ≥500 — 4xx (including 429 abuse signals) are never logged. Log 429s (at least) with IP, or amend the plan.
...
6. **[Low] Smaller items:** snapshot `SET` without `NX` (id collision would overwrite; ~62 bits of entropy makes it negligible, but `NX` + one retry is nearly free); slug-rollback failure swallowed with bare `catch {}` (at minimum `console.error`); `validateReference` accepts any charset up to 80 chars (real refs are `[a-z0-9-]{3,40}` or 12-char ids); no fetch timeout to Upstash (`AbortSignal.timeout`); 405s lack `Allow` headers; `schemaVersion`/`appDataVersion` are written but never read (use them for a migration story — relevant now that read-path validation can reject drifted snapshots — or drop them); `handleApiError`'s dual 2-arg/3-arg signature only exists for tests; `SLUG_REGEX` duplicated between client and server.
...
- **`SortableTable` — zero tests anywhere** (it's mocked out in `DonorList.test.jsx`), despite containing the bespoke negative-cost-per-life comparator (core domain logic) and the layout-measurement machinery. A pure-function comparator test would have caught §2.1.2.
...
1. **[High] `package.json` is still the template's:** `name: "webtemplate"`, `version: 1.0.0`, `main: "index.js"` (doesn't exist), empty description/author, `license: "ISC"` **with no LICENSE file** while the README recruits contributors. Add `"private": true` (publish guard) and `engines` (Node 24 to match CI).
2. **[High] `vercel` (the full CLI, 19MB) is a production dependency** with no runtime import — it accounts for 36 of 38 `npm audit --omit=dev` advisories. Move to devDependencies.
3. **[High] `prop-types` is a phantom dependency** — imported in 55 source files but not declared; it resolves only transitively (recharts → react-smooth). A dependency-tree change breaks the build. Declare it.
4. **[High] `react-router-dom` 7.13.0 carries high-severity advisories** (open-redirect relevant to SPAs; the RCE one targets framework/SSR mode). Fix is within the existing range: `^7.15`.
...
| # | Item | Refs | Size |
|---|------|------|------|
| 1 | package.json surgery: real name/description, `private: true`, `engines`, LICENSE file, `vercel` → devDeps, declare `prop-types`, bump `react-router-dom` ≥7.15 | §2.9.1–4 | S |
| 2 | Quick correctness batch: calculator sort columns + loud comparator; delete `.defaultProps`; guard persisted `JSON.parse` (clear + notify); 404 route + NotFound for unknown entity IDs; fix RecipientDetail date sort; narrow the two bare catches | §2.1.2–5, 8–9 | M |
| 3 | **Decide pulse effects** (feature or dead code) and resolve `effectsCalculation.js` accordingly — *needs Elliot* | §2.1.1 | S–M |
| 4 | Server hardening batch: generic 5xx messages, `Cache-Control` on snapshot GETs, read/health rate limit, log 429s, reject non-JSON content types + handle string bodies, Upstash fetch timeout | §2.2.1–4 | M |
...
| 14 | Test backfill: SortableTable comparator, DonationCalculator persistence, e2e donor flow + e2e in CI (nightly), re-enable jsx-a11y lint | §2.8, 2.7.5 | M |
...
Items 1–4 are independent and safe to do in any order. Items 5–7 touch the assumptions editor surface — coordinate with any in-flight feature work there. Item 9 is the deepest structural change and pays for items it overlaps with; do it before adding more content validation piecemeal.
