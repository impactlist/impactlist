...
`ImpactValueCell` is THE lives/cost-per-life table cell (∞ renders exactly for ±Infinity — the domain sentinel; NaN/0 deliberately render loud — and don't reintroduce `=== 0` checks). `ListPageShell` is the list-page scaffold, and `ListSearchControls` is the search-box + assumptions-selector toolbar row above the DonorList/RecipientList tables (pairs with `hooks/useNameSearch`). `CurrencyInput`/`NumericInput` share their cursor/comma core via `hooks/useFormattedNumberInput`; `FormField` deliberately does NOT use that hook — it is fully controlled (parent owns the draft string, percentage ×100 display transform) and only shares the formatter utilities.
