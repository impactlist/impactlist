...
`ImpactValueCell` is THE lives/cost-per-life table cell (∞ renders exactly for ±Infinity — the domain sentinel; NaN/0 deliberately render loud — and don't reintroduce `=== 0` checks). `ListPageShell` is the list-page scaffold, and `ListSearchControls` is the search-box + assumptions-selector toolbar row above list tables; its optional `filterControl` slot hosts DonorList's responsive `CauseFilter` without duplicating toolbar layout. `CauseFilter` stages checkbox changes, uses an anchored dialog on desktop and `ModalShell` on mobile, and pairs with `hooks/useCauseFilter` for applied URL state. `CurrencyInput`/`NumericInput` share their cursor/comma core via `hooks/useFormattedNumberInput`; `FormField` deliberately does NOT use that hook — it is fully controlled (parent owns the draft string, percentage ×100 display transform) and only shares the formatter utilities.
