    ...
    },
    {
      ...
      render: (donation) => {
        ...
        if (donation.isUnknown) {
          return <div className="text-sm text-slate-500">{donation.categoryName}</div>;
        }

        // Get the recipient to check if it has multiple categories
        const recipientDetails = getRecipientById(donation.recipientId);
        if (!recipientDetails) {
          return <div className="text-sm text-slate-900">{donation.categoryName}</div>;
        }
        ...
        return (
          <div className="text-sm text-slate-900">
            ...
            {hasMultipleCategories && <span className="text-xs text-slate-500 ml-1">(+{categoryCount - 1})</span>}
          </div>
        );
      },
    },
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
            ...
            <div className="flex flex-col items-center p-4 bg-slate-50 rounded-lg">
              ...
              {donorStats.totalDonatedField && (
                <span className="text-xs text-slate-500 mt-1">{formatCurrency(donorStats.knownDonations)} known</span>
              )}
            </div>
            <div className="flex flex-col items-center p-4 bg-slate-50 rounded-lg">
              ...
              <span className={`text-3xl font-bold ${donorStats.costPerLife < 0 ? 'text-red-600' : 'text-slate-900'}`}>
                {donorStats.livesSaved === 0 ? (
                  ...
                  formatCurrency(donorStats.costPerLife)
                )}
              </span>
            </div>
            <div className="flex flex-col items-center p-4 bg-slate-50 rounded-lg">
