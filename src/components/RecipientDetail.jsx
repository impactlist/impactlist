        ...
        <div className="bg-white p-3 shadow-md rounded-md border border-slate-200">
          ...
          {chartView === 'donations' ? (
            <>
              ...
              <p className="text-xs text-slate-500">{`${percentage}% of total donations`}</p>
              <div className="mt-1 pt-1 border-t border-slate-100">
                <p className="text-xs text-slate-600">Cost per life: {formatCurrency(costPerLife || 0)}</p>
                <p className="text-xs text-slate-600">Lives saved: {formatNumber(Math.round(entry.livesSavedValue))}</p>
              </div>
            </>
          ) : (
            <>
    ...
    >
      ...
      >
        ...
        >
          ...
          <div
            className={`grid grid-cols-1 ${recipientInfo.categoryBreakdown.length === 1 ? 'md:grid-cols-4' : 'md:grid-cols-3'} gap-6`}
          >
            ...
            <div className="flex flex-col items-center p-4 bg-slate-50 rounded-lg">
              ...
              <span
                className={`text-3xl font-bold ${recipientInfo.totalLivesSaved < 0 ? 'text-red-600' : 'text-emerald-600'}`}
              >
            ...
            <div className="flex flex-col items-center p-4 bg-slate-50 rounded-lg">
              ...
              <span
                className={`text-3xl font-bold ${recipientInfo.costPerLife < 0 ? 'text-red-600' : 'text-slate-900'}`}
              >
                {recipientInfo.costPerLife === 0 ? (
                  <span className="text-6xl">∞</span>
                ) : (
                  formatCurrency(recipientInfo.costPerLife)
                )}
              </span>
              <span className="text-sm mt-2 text-slate-500">
                Category avg:{' '}
                {recipientInfo.categoryCostPerLife === 0 ? (
                  <span className="text-xl">∞</span>
                ) : (
                  formatCurrency(recipientInfo.categoryCostPerLife)
                )}
              </span>
            </div>
            ...
            {recipientInfo.categoryBreakdown.length === 1 && (
              <div className="flex flex-col items-center p-4 bg-slate-50 rounded-lg">
                ...
                <span className="text-3xl font-bold text-slate-900">{recipientInfo.categoryBreakdown[0].name}</span>
              </div>
            )}
          </div>
        </motion.div>
                ...
                {chartData.length > 0 ? (
                  <ImpactBarChart
                    ...
                    labelFormatter={(value, entry) => {
                      ...
                      const percentage =
                        chartView === 'donations' ? entry.donationPercentage : entry.livesSavedPercentage;
