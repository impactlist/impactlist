...
- `AssumptionsPage` — the editor shell. Tab/entity state lives in the URL (`?tab=...&categoryId=...`) with deliberate push/replace semantics and back/forward support; it's the best-tested page (50+ integration tests) — run them after any change to save/share/library flows. Two dirtiness indicators coexist BY DESIGN: the library panel reports only the APPLIED state vs the active entry (what the whole site renders), while un-applied form drafts show only as the editor's enabled Apply (preserved across entry switches by `useGlobalForm`'s value-aware rehydration, loss-guarded by the navigation blocker). Don't make the panel draft-aware — it would misreport the applied state; the two-indicator contract is pinned by the "preserves unapplied global drafts" tests.
