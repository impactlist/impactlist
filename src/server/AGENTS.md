...
- This validator is deliberately mirrored by the client-side `normalizeUserAssumptions` checks in `src/utils/assumptionsAPIHelpers.js`; change one, change both (unification is review item 9). Same for `sharedAssumptionsNormalization.js`, which near-duplicates the client normalize.
